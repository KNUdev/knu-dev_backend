...
public class AdminTaskUploadController {
    ...
    @PostMapping(value = "/campus/{role}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public void uploadTaskForRole(
            @PathVariable("role") AccountRole accountRole,
            @RequestParam("taskFile") @Valid @NotNull MultipartFile taskFile) {
        taskUploadAPI.uploadTaskForRole(accountRole, taskFile);
    }
}
